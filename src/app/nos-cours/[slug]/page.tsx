                ...
                {course.sessions.map((session) => (
                  ...
                  >
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {session.note ? (
                        <span className="rounded bg-gold/10 px-2.5 py-1 text-xs font-bold text-stone-800">
                          {session.note}
                        </span>
                      ) : null}
                    </div>
                    ...
                    <dl className="mt-4 grid gap-3 text-sm text-stone-700 sm:grid-cols-2">
                      ...
                      {session.restartDate ? (
                        <div>
                          <dt className="font-black text-ink">Reprise</dt>
                          <dd className="mt-1">Le {session.restartDate}</dd>
                        </div>
                      ) : null}
                      <div>
          ...
          <aside className="rounded-lg bg-stone-50 p-5">
            <dl className="grid gap-4 text-sm">
              ...
              <div>
                ...
                <dd className="mt-1 grid gap-1 text-stone-600">
                  {[
                    ...new Set(
                      course.sessions.flatMap((session) => session.monitors),
                    ),
                  ].map((monitor) => (
            ...
            {course.registrationUrl ? (
              <Link
                ...
                className="mt-6 block rounded bg-brand px-4 py-3 text-center font-bold text-white hover:bg-brand-dark"
              >
                S'inscrire
              </Link>
            ) : null}
