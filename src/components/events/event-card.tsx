  ...
  return (
    <article className="grid gap-4 rounded-lg border border-stone-200 bg-white p-5 shadow-soft">
      ...
      <div>
        ...
        <p className="mt-2 text-sm leading-6 text-stone-600">{item.location}</p>
        {"description" in item ? <p className="mt-3 text-sm leading-6 text-stone-600">{item.description}</p> : null}
        {"score" in item && item.score ? <p className="mt-3 font-black text-ink">Résultat: {item.score}</p> : null}
      </div>
      <div className="flex flex-wrap gap-2">
        {item.href ? (
          <Link href={item.href} className="rounded bg-brand px-4 py-2 text-sm font-bold text-white hover:bg-brand-dark">
            Détails
          </Link>
        ) : null}
        {"programUrl" in item && item.programUrl ? (
          <Link href={item.programUrl} className="rounded border border-stone-300 px-4 py-2 text-sm font-bold">
            Programme
          </Link>
        ) : null}
        {"resultsUrl" in item && item.resultsUrl ? (
          <Link href={item.resultsUrl} className="rounded border border-stone-300 px-4 py-2 text-sm font-bold">
            Résultats
          </Link>
        ) : null}
      </div>
